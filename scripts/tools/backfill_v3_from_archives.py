...
class HistoricalBackfillProcessor:
    ...
    def __init__(
        ...
        self.db_project_name = (
            db_name
            if db_name
            else (project_name[:-4] if project_name.endswith("_raw") else project_name)
        )
        self.db_path = (
            base_dir
            / "data"
    ...
    def run(self):
        ...
        print(
            f"Mode: {'DRY RUN (preview only)' if self.dry_run else 'EXECUTE (will write database)'}"
        )
        print("=" * 80)
        print()
        ...
        if len(original_images) != project_metadata["starting_images"]:
            print(
                f"[!] WARNING: Original count ({len(original_images)}) doesn't match timesheet ({project_metadata['starting_images']})"
            )
        if len(cropped_images) != project_metadata["finished_images"]:
            print(
                f"[!] WARNING: Cropped count ({len(cropped_images)}) doesn't match timesheet ({project_metadata['finished_images']})"
            )
        ...
        if (
            ...
            and self.stats["groups_found"] == 0
            and self.min_group_size > 1
        ):
            print(
                "[*] No groups found with current min_group_size; retrying with singleton groups (min_group_size=1)..."
            )
        ...
        self.stats["groups_matched"] = len(
            [g for g in matched_groups if g["matched_cropped"]]
        )
        self.stats["groups_approved"] = len(
            [g for g in matched_groups if not g["matched_cropped"]]
        )
    ...
    def _load_project_metadata(self) -> dict | None:
        ...
        try:
            ...
            for i in range(project_row + 1, len(rows)):
                ...
                if len(next_row) > 4 and next_row[4]:  # Next project found
                    break
                if (
                    len(next_row) > 3 and next_row[3]
                ):  # Has hours, same project continues
    ...
    def _group_images(self, images: list[Path]) -> list[list[Path]]:
        ...
        groups = find_consecutive_stage_groups(
            sorted_images, min_group_size=self.min_group_size
        )

        return groups

    def _match_cropped_to_groups(
        self, groups: list[list[Path]], cropped_images: list[Path]
    ) -> list[dict]:
    ...
    def _extract_crop_coordinates(
        self, matched_groups: list[dict], metadata: dict
    ) -> list[dict]:
        ...
        for i, group_data in enumerate(matched_groups):
            ...
            decision = {
                "group_id": group_id,
                "timestamp": f"{metadata['start_date']}T12:00:00Z"
                if "T" not in str(metadata["start_date"])
                else str(metadata["start_date"]),
                ...
                if coords and confidence > 0.8:
                    ...
                    if _is_full_frame(x1, y1, x2, y2):
                        ...
                        decision["crop_timestamp"] = (
                            f"{metadata['end_date']}T12:00:00Z"
                            if "T" not in str(metadata["end_date"])
                            else str(metadata["end_date"])
                        )
                    ...
                    print(
                        f"    [!] Low confidence ({confidence:.2f}) for {original_img.name}"
                    )
    ...
    def _find_crop_coordinates(
        self, original_path: Path, cropped_path: Path
        ...
        try:
            ...
            if self.db_path.exists():
                ...
                trash_path = (
                    Path.home()
                    / ".Trash"
                    / f"{self.db_path.stem}_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}{self.db_path.suffix}"
                )
                try:
            ...
            # Create indexes
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_project_id ON ai_decisions(project_id)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_timestamp ON ai_decisions(timestamp)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_user_action ON ai_decisions(user_action)"
            )
            ...
            for decision in decisions:
                ...
                if width == 0 or height == 0:
                    try:
                        img_path = (
                            self.originals_dir
                            / decision["images"][decision["user_selected_index"]]
                        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Backfill v3 training data from historical archives"
    )
    parser.add_argument(
        "project_name", help="Project name (used for defaults and DB name)"
    )
    parser.add_argument(
        "--execute", action="store_true", help="Execute (default is dry-run)"
    )
    parser.add_argument(
        "--base-dir", default=".", help="Base directory (default: current)"
    )
    parser.add_argument(
        "--raw-dir", default=None, help="Explicit RAW/originals directory"
    )
    parser.add_argument(
        "--final-dir",
        default=None,
        help="Explicit FINAL/cropped directory (suffix _final is common)",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
        help="Recursively scan RAW/FINAL subdirectories for images",
    )
    parser.add_argument(
        "--min-group-size",
        type=int,
        default=2,
        help="Minimum group size (use 1 to allow singletons)",
    )
    parser.add_argument(
        "--allow-singleton-fallback",
        action="store_true",
        help="If no groups found, retry with min-group-size=1",
    )
    parser.add_argument(
        "--replace-db",
        action="store_true",
        help="Move existing DB to Trash, then write a new one",
    )
    parser.add_argument(
        "--db-name",
        default=None,
        help="Override DB project name (default strips _raw suffix)",
    )
    parser.add_argument(
        "--manifest",
        default=None,
        help="Path to project manifest JSON (preferred source of truth)",
    )
