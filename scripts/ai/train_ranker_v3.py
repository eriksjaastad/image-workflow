...
def load_training_data(
    embeddings_cache: dict, anomaly_set: set
    ...
    def load_embedding(self, image_path: str) -> np.ndarray:
        ...
        if image_path in self.embeddings_cache:
            ...
            if filename not in filename_cache:
                msg = f"No embedding found for {image_path} (or filename {filename})"
                raise ValueError(msg)
        ...
        if not emb_file.exists():
            msg = f"Embedding file not found: {emb_file}"
            raise ValueError(msg)
...
def validate(model, dataloader, criterion, device):
    ...
    overall_acc = (
        (normal_correct + anomaly_correct) / (normal_total + anomaly_total)
        if (normal_total + anomaly_total) > 0
        else 0
    )
...
def main():
    ...
    val_dataset = RankingDataset(
        normal_val, anomaly_val, embeddings_cache, anomaly_oversample_factor=1
    )  # No oversampling in val
    ...
    criterion = WeightedMarginRankingLoss(
        margin=0.5, anomaly_weight=args.anomaly_weight
    )
