...
def parse_timestamp_to_utc(ts_str: str) -> datetime | None:
...
def count_png_files(record: dict[str, Any]) -> tuple[int, int]:
    ...
    if files_list:
        png_count = sum(
            1 for f in files_list if isinstance(f, str) and f.lower().endswith(".png")
        )
...
def load_raw_logs_for_day(data_dir: Path, day_str: str) -> list[dict[str, Any]]:
    ...
    # Try archived logs
    archive_path = (
        data_dir / "log_archives" / f"file_operations_{day_str}_archived.log.gz"
    )
...
def aggregate_to_bins(
    records: list[dict[str, Any]], bin_version: int = 1
) -> list[dict[str, Any]]:
    ...
    bins: dict[tuple, dict[str, Any]] = defaultdict(
        lambda: {"events": [], "file_count": 0, "file_count_total": 0, "event_count": 0}
    )
    ...
    for key, data in bins.items():
        ...
        first_event_ts = (
            min(event_timestamps).isoformat() if event_timestamps else bin_ts_str
        )
        last_event_ts = (
            max(event_timestamps).isoformat() if event_timestamps else bin_ts_str
        )
...
def aggregate_day(
    data_dir: Path, day_str: str, bin_version: int = 1, dry_run: bool = False
) -> int:
