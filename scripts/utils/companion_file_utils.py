...
def find_consecutive_stage_groups(
    ...
    while i < n:
        ...
        while i < n:
            ...
            steps = 0
            while (
                k < n and steps < lookahead and not boundary(prev_s, prev_dt, files[k])
            ):
                s = stage_of(files[k])
                if s > prev_s:
            ...
            i = (
                chosen_k + 1
            )  # advance past chosen; earlier larger stages are intentionally skipped
...
def find_mismatched_files(directory: Path) -> dict[str, list[Path]]:
    ...
    image_files = [
        f for f in all_files if f.is_file() and f.suffix.lower() in image_extensions
    ]
    metadata_files = [
        f for f in all_files if f.is_file() and f.suffix.lower() in metadata_extensions
    ]
    ...
    return {"orphaned_images": orphaned_images, "orphaned_metadata": orphaned_metadata}
        ...
        if not dry_run:
            try:
                ...
                moved_files = move_file_with_all_companions(
                    image_file, dest_dir, dry_run=False
                )

                # Log the operations if tracker provided (image-only count)
                if tracker:
                    pngs = [
                        str(f) for f in moved_files if str(f).lower().endswith(".png")
                    ]
            ...
            moved_files = move_file_with_all_companions(
                image_file, dest_dir, dry_run=True
            )
            moved_count += 1
...
def log_select_crop_entry(
    ...
    row = {
        ...
        if 0 <= chosen_index < len(image_paths)
        else "",
...
def format_image_display_name(
    filename: str, max_length: int = 30, context: str = "web"
) -> str:
    """
...
def calculate_work_time_from_file_operations(
    ...
    Rules:
    - Time is binned into 15-minute intervals.
    - We treat gaps between consecutive file-ops <= 120 seconds as active work and
      accumulate those seconds into the appropriate 15-minute bin(s).
    - A bin only counts if it has at least 600 seconds (10 minutes) of active time.
      Qualified bins contribute 900 seconds (15 minutes) each to the total.

    This prevents sparse operations from inflating hours while still crediting
    consolidated work sessions.

    Args:
        file_operations: List of file operation dictionaries from FileTracker
        break_threshold_minutes: DEPRECATED (kept for signature compatibility)
    ...
    BIN_SECS = 15 * 60
    ACTIVE_THRESHOLD = int(7.5 * 60)  # 7.5 minutes required to count a bin
    GAP_SECS = 300  # 5 minutes; larger gaps are idle
    ...
    for cur in stamps[1:]:
        ...
        if gap <= GAP_SECS:
            ...
            while seg_start < seg_end:
                bin_start = floor_to_bin(seg_start)
                bin_end = bin_start + _td(seconds=BIN_SECS)
                chunk_end = min(seg_end, bin_end)
                active = (chunk_end - seg_start).total_seconds()
                if active > 0:
                    bin_active[bin_start] = bin_active.get(bin_start, 0.0) + active
                seg_start = chunk_end

        prev = cur

    # Count bins that meet either activity-time threshold OR file-count threshold
    FILES_THRESHOLD = 30  # counts a bin when ≥30 files processed in the bin
    all_bins = set(bin_active.keys()) | set(bin_files.keys())
    qualified_bins = 0
    for b in all_bins:
        active_ok = bin_active.get(b, 0.0) >= ACTIVE_THRESHOLD
        files_ok = bin_files.get(b, 0) >= FILES_THRESHOLD
        if active_ok or files_ok:
            qualified_bins += 1
    ...
    if len(sorted_ops) >= 2:
        try:
            ...
            end_time = datetime.fromisoformat(
                sorted_ops[-1]["timestamp"].replace("Z", "+00:00")
            )
