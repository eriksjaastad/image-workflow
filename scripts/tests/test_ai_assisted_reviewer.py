    ...
    def load_ai_models(models_dir: Path):
        ...
        try:
            ...
            if crop_path.exists():
                ...
                crop_proposer.load_state_dict(
                    torch.load(crop_path, map_location=device)
                )
                crop_proposer.eval()
            else:
                crop_proposer = None
...
def test_model_loading():
    ...
    assert isinstance(
        crop_model, CropProposerModel
    ), "Crop model should be CropProposerModel instance"
...
def test_rule_based_recommendation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        group = ImageGroup(group_id="20250101_120000", images=images, directory=tmpdir)

        recommendation = get_rule_based_recommendation(group)

        # Should pick highest stage (stage2 = last index)
        assert (
            recommendation["selected_index"] == len(images) - 1
        ), f"Should pick last image (highest stage), got {recommendation['selected_index']}"
...
def test_crop_coordinates_validation():
    ...
    for i, coords in enumerate(test_coords):
        ...
        print(
            f"   ✅ Coords {i}: ({x1:.2f}, {y1:.2f}, {x2:.2f}, {y2:.2f}) - Area: {crop_area:.2%}"
        )
...
def test_crop_aspect_ratio_preservation():
    ...
    for img_w, img_h, coords, desc in test_cases:
        ...
        print(
            f"   ✅ {desc.capitalize()}: {img_w}x{img_h} → crop {crop_w:.0f}x{crop_h:.0f}"
        )
...
def test_batch_size_configuration():
    ...
    for batch_size in test_batch_sizes:
        ...
        ), f"Should have {expected_batches} batches"
        ...
        print(
            f"   ✅ Batch size {batch_size}: {len(batches)} batches for {total_groups} groups"
        )
