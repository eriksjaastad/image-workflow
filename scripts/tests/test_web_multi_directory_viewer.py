...
class WebMultiDirectoryViewerTest:
    ...
    def setup(self):
        ...
        cmd = [
            ...
            "--port",
            "5004",  # Use different port for tests
            "--no-browser",
        ]
    ...
    def test_page_loads(self):
        ...
        directory_sections = self.driver.find_elements(
            By.CSS_SELECTOR, ".directory-section"
        )
        images = self.driver.find_elements(By.TAG_NAME, "img")

        assert (
            len(directory_sections) > 0 or len(images) > 0
        ), "Should display directories or images"
    ...
    def test_sticky_header_with_stats(self):
        ...
        assert (
            "0 delete" in delete_count.text
        ), f"Initial delete count should be 0, got {delete_count.text}"
        assert (
            "0 crop" in crop_count.text
        ), f"Initial crop count should be 0, got {crop_count.text}"
    ...
    def test_image_click_delete_toggle(self):
        ...
        if len(images) > 0:
            ...
            delete_selected = self.driver.find_elements(
                By.CSS_SELECTOR, ".image-container.delete-selected"
            )
            assert (
                len(delete_selected) > 0
            ), "Image should have delete state after click"

            # Check that delete count updated
            delete_count = self.driver.find_element(By.ID, "deleteCount")
            assert (
                "1 delete" in delete_count.text
            ), f"Delete count should be 1, got {delete_count.text}"
            ...
            assert (
                len(delete_selected) == 0
            ), "Image should not have delete state after second click"

            # Check that delete count reset
            delete_count = self.driver.find_element(By.ID, "deleteCount")
            assert (
                "0 delete" in delete_count.text
            ), f"Delete count should be 0, got {delete_count.text}"
    ...
    def test_crop_button_functionality(self):
        ...
        if len(crop_buttons) > 0:
            ...
            crop_selected = self.driver.find_elements(
                By.CSS_SELECTOR, ".image-container.crop-selected"
            )
            assert (
                len(crop_selected) > 0
            ), "Image should have crop state after crop button click"

            # Check that crop count updated
            crop_count = self.driver.find_element(By.ID, "cropCount")
            assert (
                "1 crop" in crop_count.text
            ), f"Crop count should be 1, got {crop_count.text}"
            ...
            assert (
                len(crop_selected) == 0
            ), "Image should not have crop state after second click"

            # Check that crop count reset
            crop_count = self.driver.find_element(By.ID, "cropCount")
            assert (
                "0 crop" in crop_count.text
            ), f"Crop count should be 0, got {crop_count.text}"
    ...
    def test_image_aspect_ratios(self):
        ...
        if len(images) > 0:
            ...
            assert (
                max_width == "100%"
            ), f"Images should have max-width: 100%, got {max_width}"
            assert (
                "300px" in max_height
            ), f"Images should have max-height: 300px, got {max_height}"
    ...
    def test_style_guide_compliance(self):
        ...
        assert (
            danger_color.strip() == "#ff6b6b"
        ), f"Danger color should be #ff6b6b, got {danger_color}"
