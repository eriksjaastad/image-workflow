...
def create_mixed_test_data(
    output_dir, triplets=100, pairs=10, singletons=5, with_subdirs=False
):
    ...
    print(
        f"🔧 Creating mixed test data: {triplets} triplets, {pairs} pairs, {singletons} singletons{subdirs_msg}"
    )
...
def create_subdirectory_test_data(output_dir):
    ...
    print(
        f"   📁 Including {len(random_files) + len(problematic_files)} non-standard files that should be ignored"
    )
    return files_created
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create test data for image selector testing"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    parser.add_argument(
        "--triplets", "-t", type=int, default=120, help="Number of triplets to create"
    )
    parser.add_argument(
        "--pairs", "-p", type=int, default=0, help="Number of pairs to create"
    )
    parser.add_argument(
        "--singletons", "-s", type=int, default=0, help="Number of singletons to create"
    )
    parser.add_argument(
        "--size",
    ...
    parser.add_argument(
        "--from-manifest", help="Create test data based on existing manifest"
    )
    ...
    try:
        if args.from_manifest:
            ...
            files = create_performance_test_data(
                args.output, args.size, args.with_subdirs
            )
        elif args.pairs > 0 or args.singletons > 0:
            files = create_mixed_test_data(
                args.output,
                args.triplets,
                args.pairs,
                args.singletons,
