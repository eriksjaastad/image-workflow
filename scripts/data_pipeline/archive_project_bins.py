...
class ProjectArchiver:
    ...
    def load_project_manifest(self, project_id: str) -> dict[str, Any] | None:
    ...
    def collect_project_bins(
        self, project_id: str, started_at: str, finished_at: str
    ) -> list[dict[str, Any]]:
    ...
    def create_project_summary(
        self, project_id: str, manifest: dict[str, Any], bins: list[dict[str, Any]]
    ) -> dict[str, Any]:
    ...
    def write_project_archive(
        ...
        with tempfile.NamedTemporaryFile(
            mode="w", dir=archive_dir, delete=False, suffix=".tmp"
        ) as tmp_file:
    ...
    def merge_into_overall(
        self, project_id: str, bins: list[dict[str, Any]], dry_run: bool = False
    ) -> None:
        ...
        # Write atomically
        with tempfile.NamedTemporaryFile(
            mode="w", dir=self.overall_dir, delete=False, suffix=".tmp"
        ) as tmp_file:
            tmp_path = Path(tmp_file.name)
...
def main():
    ...
    parser.add_argument("project_id", type=str, help="Project ID to archive")
    parser.add_argument(
    ...
    if args.rollback:
        ...
        success = archiver.archive_project(
            args.project_id, dry_run=args.dry_run, skip_merge=args.skip_merge
        )
