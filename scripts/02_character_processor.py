...
**Step 1:** Initial character separation (emily, ivy, mia)
  python scripts/02_character_processor.py "__selected/"

**Step 2:** Subdivide character directories by demographics
  python scripts/02_character_processor.py "__selected/emily/" --group-by ethnicity
  python scripts/02_character_processor.py "__selected/emily/" --group-by age
  # Note: Hierarchical grouping (ethnicity,age) is not yet implemented

USAGE:
------
  # Full demographic breakdown (no file moves)
  python scripts/02_character_processor.py "__selected/emily/" --analyze
  ...
  python scripts/02_character_processor.py "__selected/emily/" --group-by ethnicity    # → latina/, asian/, white/
  python scripts/02_character_processor.py "__selected/emily/" --group-by age          # → mid_20s/, early_30s/
  python scripts/02_character_processor.py "__selected/emily/" --group-by body_type    # → petite/, curvy/, athletic/
...
def extract_age_from_prompt(prompt_lower: str) -> str | None:
    ...
    age_match = re.search(
        r"in\s+(?:her|his)\s+(early|mid|late)?\s*(\d+)s", prompt_lower
    )
    if age_match:
        descriptor = (age_match.group(1) or "").strip()
        decade = age_match.group(2)
        # Drop gender prefix for readability; keep descriptor if present
        return f"{descriptor + '_' if descriptor else ''}{decade}s"
...
def extract_descriptive_character_from_prompt(
    ...
    if group_by == "ethnicity":
        ...
        if ethnicity_value and (
            "@" in ethnicity_value
            or "(" in ethnicity_value
            or ")" in ethnicity_value
            or any(c.isdigit() for c in ethnicity_value)
        ):
            return None
...
def analyze_prompts_for_characters(
    ...
    print(
        f"[*] Analyzing prompts for {len(files_without_character)} files without character data..."
    )
    ...
    for filename, char_info in files_without_character:
        ...
        if prompt_char:
            prompt_character_counts[prompt_char] = (
                prompt_character_counts.get(prompt_char, 0) + 1
            )
    ...
    if not viable_characters:
        print(
            f"[*] No prompt-based character groups meet minimum threshold of {min_threshold} files"
        )
        return {}
...
def parse_caption_file(caption_path: Path) -> dict | None:
    ...
    try:
        ...
        msg = f"Caption parsing failed for {caption_path}"
        raise RuntimeError(msg) from e
...
def parse_yaml_file(yaml_path: Path) -> dict | None:
    ...
    try:
        ...
        msg = f"YAML parsing failed for {yaml_path}"
        raise RuntimeError(msg) from e
...
def analyze_yaml(
    ...
    if not quiet:
        ...
        print(
            f"[*] Found {len(yaml_files)} YAML files and {len(caption_files)} caption files"
        )
    ...
    for i, metadata_path in enumerate(metadata_files):
        ...
        if result["has_character"]:
            ...
            for char_name in result["all_characters"]:
                ...
                processing_stats["character_counts"][char_name] = (
                    processing_stats["character_counts"].get(char_name, 0) + 1
                )
        else:
    ...
    if output_file:
        ...
        try:
            ...
            msg = f"Analysis file write failed: {output_path}"
            raise RuntimeError(msg) from e
    ...
    if not quiet:
        ...
        if processing_stats["character_counts"]:
            most_common = max(
                processing_stats["character_counts"].items(), key=lambda x: x[1]
            )
            print(
...
def find_sequential_neighbors(
    ...
    # Files after
    for i in range(
        target_pos + 1, min(len(timestamped_files), target_pos + window_size + 1)
    ):
...
def add_sequential_context(analysis_data: dict, quiet: bool = False) -> dict:
    ...
    for filename in no_character_files:
        inferred_character = infer_character_from_context(
            filename, character_mapping, all_files
        )
    ...
    if not quiet:
        ...
        if inference_details:
            ...
            character_counts = Counter(
                [detail["inferred_character"] for detail in inference_details]
            )
...
def preview_grouping_plan(
    ...
    for _filename, char_info in character_mapping.items():
        ...
        is_multi_character = (
            char_info.get("is_multi_character", False) and group_by == "character"
        )
    ...
    if files_staying_in_root > 0:
        ...
        print(
            f"   • {files_staying_in_root} images (no {group_by} detected or below threshold)"
        )
...
def move_file_pair(
    ...
    try:
        ...
        msg = f"File move failed for {png_path.name}"
        raise RuntimeError(msg) from e
...
def group_by_category(
    ...
    for file_data in character_mapping.values():
        category_name = file_data.get(
            "character_name"
        )  # This field holds the extracted category value
        if category_name:
    ...
    for png_filename, char_info in character_mapping.items():
        ...
        if group_by == "character":
            ...
            has_character = bool(
                character
            )  # Has character if we have an inferred category
        ...
        if (
            group_by == "character"
            and not char_info.get("has_character", False)
            and inferred_character
        ):
            character = inferred_character
        ...
        if is_multi_character and len(all_characters) > 1 and group_by == "character":
            ...
            if target_dir is None:
                ...
                if not dry_run:
                    ...
                    if not quiet:
                        print(
                            f"[*] Created multi-character directory: {target_dir.name}/"
                        )
                elif not quiet:
                    print(
                        f"[DRY RUN] Would create multi-character directory: {target_dir.name}/"
                    )
        else:
            # Single character
        ...
        yaml_filename = char_info.get(
            "yaml_file", png_filename.replace(".png", ".yaml")
        )
        ...
        if dry_run and not quiet:
            print(
                f"[DRY RUN] Would move: {png_filename} + {yaml_filename} → {target_dir.name}/"
            )
    ...
    if not quiet:
        ...
        for char, count in sorted(
            stats["character_counts"].items(), key=lambda x: -x[1]
        ):
            if count > 0:
                print(f"  • {char}: {count} images")
        ...
        if stats["skipped_unknown"] > 0:
            source_dir_name = Path(source_directory).name
            print(
                f"  • (stayed in {source_dir_name}/): {stats['skipped_unknown']} images"
            )
...
def process_directory(
    ...
    if save_analysis:
        dir_name = (
            Path(directory).name.replace(" ", "_").replace("(", "").replace(")", "")
        )
    ...
    if group_by != "character":
        ...
        for filename, char_info in enhanced_data["character_mapping"].items():
            prompt = char_info.get("prompt", "")
            stage = char_info.get("stage", "unknown")
            category_value = extract_descriptive_character_from_prompt(
                prompt, group_by, stage
            )
        ...
        if not viable_categories:
            if not quiet:
                print(
                    f"[*] No {group_by} groups meet minimum threshold of {min_threshold} files"
                )
            ...
            for filename, category_value in category_assignments.items():
                if (
                    category_value in viable_categories
                    and filename in enhanced_data["character_mapping"]
                ):
                    enhanced_data["character_mapping"][filename]["character_name"] = (
                        category_value
                    )
                    enhanced_data["character_mapping"][filename]["has_character"] = True
                    enhanced_data["character_mapping"][filename]["source"] = (
                        f"prompt_analysis_{group_by}"
                    )
                    files_assigned += 1
            ...
            if not quiet:
                print(
                    f"[*] Extracted '{group_by}' for {files_assigned} files → {len(viable_categories)} groups (threshold: {min_threshold})"
                )
                for category, count in sorted(viable_categories.items()):
                    print(f"    • {category}: {count} files")
                if files_below_threshold > 0:
                    print(
                        f"[*] {files_below_threshold} files in groups below threshold (will stay in root directory)"
                    )
    else:
        # Original behavior: only process files without LoRA character data
        prompt_assignments = analyze_prompts_for_characters(
        ...
        if prompt_assignments:
            ...
            enhanced_data["summary"]["prompt_analysis"] = {
                ...
                "character_groups": {
                    char: list(prompt_assignments.values()).count(char)
            ...
            if not quiet:
                print(
                    f"[*] Assigned {len(prompt_assignments)} files to {len(set(prompt_assignments.values()))} prompt-based character groups"
                )
                for char, count in enhanced_data["summary"]["prompt_analysis"][
                    "character_groups"
                ].items():
                    print(f"    • {char}: {count} files")
        else:
    ...
    if save_analysis:
        context_file = (
            analysis_file.replace(".json", "_with_context.json")
            if analysis_file
            else None
        )
        if context_file:
    ...
    # Stage 3: Character Grouping
    grouping_stats = group_by_category(
        enhanced_data, directory, dry_run, quiet, group_by
    )
    ...
    results = {
        ...
        "pipeline_summary": {
            ...
            "success_rate": (
                grouping_stats["moved_successfully"] / grouping_stats["total_files"]
            )
            * 100
            if grouping_stats["total_files"] > 0
            else 0,
        },
    }
...
Examples:
  ...
  python scripts/02_character_processor.py "directory/" --save-analysis
    ...
    parser.add_argument(
        "--save-analysis",
        action="store_true",
        help="Save intermediate analysis JSON files",
    )
    parser.add_argument(
    ...
    if args.analyze:
        ...
        yaml.add_constructor(
            "tag:yaml.org,2002:python/tuple", tuple_constructor, Loader=yaml.SafeLoader
        )
            ...
            try:
                ...
                if not prompt:
                    files_with_errors.append(
                        (yaml_file, "No prompt found in YAML or caption file")
                    )
                    continue
        ...
        if files_without_ethnicity:
            print(
                f"⚠️  Files without ethnicity detected: {len(files_without_ethnicity)}"
            )
        ...
        if files_with_errors:
            ...
            display_limit = (
                20 if len(files_with_errors) > 20 else len(files_with_errors)
            )
        ...
        if hair_colors:
            print("\n💇 HAIR COLORS:")
            for hair, count in sorted(
                list(hair_colors.items())[:20], key=lambda x: -x[1]
            ):  # Top 20
        ...
        print(
            f'   python scripts/02_character_processor.py "{args.directory}" --group-by <category>'
        )
        print(
