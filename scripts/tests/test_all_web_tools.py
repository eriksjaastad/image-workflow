...
class WebToolsTestSuite:
    ...
    def run_failing_tests_only(self):
        ...
        print(
            "📝 Note: All current tests are designed to pass with the implemented features"
        )
        print(
            "   To create failing tests, we would need to test unimplemented features"
        )
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--failing":
            ...
            print(
                "Usage: python test_all_web_tools.py [--failing|--image-selector|--character-sorter|--multi-directory]"
            )
            sys.exit(1)
    else:
