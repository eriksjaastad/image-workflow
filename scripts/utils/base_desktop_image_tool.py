...
class BaseDesktopImageTool:
    ...
    def _set_selector_extents_safely(
    ...
    def on_crop_select(self, eclick, erelease, image_idx: int):
        ...
        active_aspect_ratio = (
            ...
            else self.image_states[image_idx]["image_aspect_ratio"]
        )
    ...
    def crop_and_save(self, image_info: dict, crop_coords: tuple[int, int, int, int]):
        ...
        # Detect no-op crop (full image). Skip saving to avoid mtime updates.
        is_noop_crop = x1 <= 0 and y1 <= 0 and x2 >= img_w and y2 >= img_h

        if is_noop_crop:
            # Log as crop with note to preserve pipeline expectations
    ...
    def move_to_cropped(self, png_path: Path, yaml_path: Path, reason: str):
        ...
        files = (
            [png_path.name, yaml_path.name] if yaml_path.exists() else [png_path.name]
        )
    ...
    def load_image_safely(self, png_path: Path, image_idx: int) -> bool:
        ...
        try:
            ...
            self.current_images.append(
                {"path": png_path, "image": img_array, "original_size": img.size}
            )
            ...
            ax.set_title(
                f"Image {image_idx + 1}: {format_image_display_name(png_path.name, context='desktop')}",
                fontsize=12,
            )
            ax.set_xticks([])
            ax.set_yticks([])
            ...
            if i < len(self.axes) and isinstance(state, dict):
                ...
                if status in ["keep", "selected"]:
                    self.axes[i].set_title(
                        f"Image {i + 1}: SELECTED - {format_image_display_name(self.current_images[i]['path'].name, context='desktop')}",
                        fontsize=12,
                        color="blue",
                        weight="bold",
                    )
                    ...
                    self.axes[i].set_title(
                        f"Image {i + 1}: DELETE - {format_image_display_name(self.current_images[i]['path'].name, context='desktop')}",
                        fontsize=12,
                        color="red",
                    )
                else:
                    self.axes[i].set_title(
                        f"Image {i + 1}: {status.upper()} - {format_image_display_name(self.current_images[i]['path'].name, context='desktop')}",
                        fontsize=12,
                        color="orange",
                    )
    ...
    def clear_selectors(self):
        ...
        desired = getattr(
            self, "current_num_images", len(self.axes) if self.axes else 3
        )
        try:
            lower = int(getattr(self, "min_panels", 1) or 1)
            upper = int(getattr(self, "max_panels", 4) or 4)
