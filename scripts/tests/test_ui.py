...
class ImageSelectorUITest:
    ...
    def setup(self):
        ...
        cmd = [
            ...
            "--port",
            "5002",  # Use different port for tests
            "--no-browser",
            "--batch-size",
            "3",
        ]
    ...
    def test_keyboard_shortcuts(self):
        ...
        assert "image-active" in btn1.get_attribute(
            "class"
        ), "Button 1 should be highlighted after Q key"
        assert "crop-active" in crop_btn.get_attribute(
            "class"
        ), "Crop button should be active after Q key"
        ...
        assert "image-active" in btn2.get_attribute(
            "class"
        ), "Button 2 should be highlighted after W key"
        assert "crop-active" in crop_btn.get_attribute(
            "class"
        ), "Crop button should still be active after W key"
        ...
        if btn3.is_displayed():  # Only test if image 3 exists
            body.send_keys("e")
            time.sleep(0.5)
            assert "image-active" in btn3.get_attribute(
                "class"
            ), "Button 3 should be highlighted after E key"
    ...
    def test_number_keys(self):
        ...
        assert "image-active" in btn1.get_attribute(
            "class"
        ), "Button 1 should be highlighted after 1 key"
        assert "crop-active" not in crop_btn.get_attribute(
            "class"
        ), "Crop button should NOT be active after 1 key"
    ...
    def test_button_clicks(self):
        ...
        assert "image-active" in btn1.get_attribute(
            "class"
        ), "Button 1 should be highlighted after click"
        ...
        assert "crop-active" in crop_btn.get_attribute(
            "class"
        ), "Crop button should be active after click"
    ...
    def test_visual_feedback(self):
        ...
        if images:
            ...
            assert (
                "selected" in classes
            ), f"Image should have 'selected' class, got: {classes}"
