...
class TestNearestUpGrouping(unittest.TestCase):
    ...
    def test_time_gap_functionality(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            groups_with_gap = find_consecutive_stage_groups(
                file_paths, time_gap_minutes=2
            )
    ...
    def test_lookahead_functionality(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            stages_small = [
                get_stage_number(detect_stage(f.name)) for f in groups_small[0]
            ]
            ...
            stages_large = [
                get_stage_number(detect_stage(f.name)) for f in groups_large[0]
            ]
