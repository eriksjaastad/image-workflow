        ...
        json.dump(
            {
                ...
                "total": len(records),
                "summary": {
                    "present_same_dir": sum(
                        1 for r in records if r.status == "present_same_dir"
                    ),
                    "moved_elsewhere": sum(
                        1 for r in records if r.status == "moved_elsewhere"
                    ),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Audit orphan .decision files and locate matching PNGs"
    )
    parser.add_argument(
        "--dir",
        dest="target_dir",
        default="mojo3/faces",
        help="Directory to scan for .decision files",
    )
