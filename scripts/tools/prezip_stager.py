...
def load_allowlist(
    project_id: str, allowlist_json: Path | None
...
def load_bans(bans_json: Path | None) -> tuple[set[str], list[re.Pattern], set[str]]:
    ...
    for dirpath, dirnames, filenames in os.walk(cfg.content_dir):
        ...
        for name in filenames:
            ...
            if not cfg.quiet and scanned_files % 5000 == 0:
                print(
                    f"[scan] scanned={scanned_files} eligible={len(eligible)}",
                    file=sys.stderr,
                    flush=True,
                )
    ...
    if do_companion_checks:
        ...
        if companion_issues:
            ...
            if cfg.strict_companions:
                return report | {
                    "status": "error",
                    "message": "Companion integrity check failed",
                }
    ...
    with zipfile.ZipFile(out_zip, "w", **zip_kwargs) as zf:
        for p in eligible:
            ...
            if not cfg.quiet and zipped_count % 1000 == 0:
                print(
                    f"[zip] {zipped_count}/{total_to_zip}", file=sys.stderr, flush=True
                )
...
def parse_args() -> StagerConfig:
    ...
    ap.add_argument(
        "--bans-json", default=str(Path("data/projects/global_bans.json").resolve())
    )
    ...
    return StagerConfig(
        ...
        allowlist_json=Path(args.allowlist_json).resolve()
