...
class WebImageSelectorTest:
    ...
    def setup(self):
        ...
        cmd = [
            ...
            "--port",
            "5001",  # Use different port for tests
            "--no-browser",
            "--batch-size",
    ...
    def test_keyboard_shortcuts(self):
        ...
        selected_images = self.driver.find_elements(
            By.CSS_SELECTOR, "figure.image-card.selected"
        )
        ...
        crop_selected = self.driver.find_elements(
            By.CSS_SELECTOR, "figure.image-card.crop-selected"
        )
        assert (
            len(crop_selected) > 0
        ), "Selected images should automatically have selected state"
    ...
    def test_unselect_functionality(self):
        ...
        selected_image = self.driver.find_element(
            By.CSS_SELECTOR, "figure.image-card.selected img"
        )
    ...
    def test_selection_consistency(self):
        ...
        assert (
            len(crop_selected) > 0
        ), "Selected image should automatically have selected state"
        assert len(selected_images) == len(
            crop_selected
        ), "All selected images should have selected state"
    ...
    def test_button_toggle(self):
        ...
        ), f"Should be selected after first press, found {len(selected_images)} selected images"
    ...
    def test_process_button_safety(self):
        ...
        assert (
            process_button.get_attribute("disabled") is not None
        ...
        assert (
            process_button.get_attribute("disabled") is None
        ), "Process button should be enabled after scrolling"
