...
class TestDashboardCoreFunctionality(unittest.TestCase):
    ...
    def test_data_engine_can_aggregate_by_script(self):
        ...
        # Test aggregation
        aggregated = engine.aggregate_by_time_slice(
            records, "D", "file_count", "script"
        )
        ...
        if aggregated:
            ...
            self.assertIn(
                "time_slice", agg_record, "Aggregated record should have time_slice"
            )
            self.assertIn("script", agg_record, "Aggregated record should have script")
            self.assertIn(
                "file_count", agg_record, "Aggregated record should have file_count"
            )
    ...
    def test_data_engine_can_aggregate_by_operation(self):
        ...
        # Test aggregation
        aggregated = engine.aggregate_by_time_slice(
            records, "D", "file_count", "operation"
        )
        ...
        if aggregated:
            ...
            self.assertIn(
                "operation", agg_record, "Aggregated record should have operation"
            )
            self.assertIn(
                "file_count", agg_record, "Aggregated record should have file_count"
            )
    ...
    def test_dashboard_can_generate_data(self):
        ...
        self.assertIn(
            "by_script", data["file_operations_data"], "Should have by_script data"
        )
        self.assertIn(
            "by_operation",
            data["file_operations_data"],
            "Should have by_operation data",
        )
            ...
            with open(ops_dir / "ops.log", "w") as f:
                f.write(
                    json.dumps(
                        {
                            "type": "file_operation",
                            "timestamp": "2025-03-31T23:50:00Z",
                            "operation": "move",
                            "file_count": 1,
                        }
                    )
                ...
                f.write(
                    json.dumps(
                        {
                            "type": "file_operation",
                            "timestamp": "2025-04-01T00:05:00Z",
                            "operation": "move",
                            "file_count": 1,
                        }
                    )
    ...
    def test_dashboard_can_transform_charts(self):
        ...
        # Generate raw data
        data = dashboard.data_engine.generate_dashboard_data(
            time_slice="D", lookback_days=7
        )
        ...
        self.assertIn(
            "by_operation", chart_data["charts"], "Should have by_operation charts"
        )
    ...
    def test_dashboard_has_historical_data(self):
        ...
        self.assertGreater(
            len(september_dates), 0, "Should have historical data from September"
        )

        # Should have data from October (recent)
        october_dates = [d for d in dates if d.startswith("2025-10")]
        self.assertGreater(
            len(october_dates), 0, "Should have recent data from October"
        )
    ...
    def test_dashboard_has_multiple_scripts(self):
        ...
        for expected_script in expected_scripts:
            if expected_script in scripts:
                self.assertIn(
                    expected_script, scripts, f"Should have data from {expected_script}"
                )
    ...
    def test_dashboard_has_multiple_operations(self):
        ...
        for expected_op in expected_operations:
            if expected_op in operations:
                self.assertIn(
                    expected_op, operations, f"Should have {expected_op} operations"
                )
    ...
    def test_consolidation_script_exists_and_runnable(self):
        ...
        # Should exist
        self.assertTrue(
            consolidation_script.exists(), "Consolidation script should exist"
        )
    ...
    def test_daily_summaries_exist(self):
        ...
        # Should exist
        self.assertTrue(
            summaries_dir.exists(), "Daily summaries directory should exist"
        )
        ...
        for summary_file in summary_files:
            ...
            self.assertIn(
                "total_operations", summary, "Summary should have total_operations"
            )
...
class TestDashboardDataIntegrity(unittest.TestCase):
    ...
    def test_dashboard_performance(self):
        ...
        self.assertLess(
            load_time, 5.0, f"Data loading should be fast, took {load_time:.2f}s"
        )
    ...
    def test_chart_data_structure(self):
        ...
        if "by_script" in charts:
            ...
            for script_name, script_data in by_script.items():
                self.assertIn(
                    "dates", script_data, f"Script {script_name} should have dates"
                )
                self.assertIn(
                    "counts", script_data, f"Script {script_name} should have counts"
                )
                self.assertEqual(
                    len(script_data["dates"]),
                    len(script_data["counts"]),
        ...
        if "by_operation" in charts:
            ...
            for operation_name, operation_data in by_operation.items():
                self.assertIn(
                    "dates",
                    operation_data,
                    f"Operation {operation_name} should have dates",
                )
                self.assertIn(
                    "counts",
                    operation_data,
                    f"Operation {operation_name} should have counts",
                )
                self.assertEqual(
                    len(operation_data["dates"]),
                    len(operation_data["counts"]),
