...
Examples:
    ...
    if triplets["quads"] > 0:
        print(
            f"  ⚠️  Found {triplets['quads']} quad sequences - consider updating tools to handle 4-image sequences"
        )
    ...
    if triplets["incomplete_triplets"] > 0:
        print(
            f"  ⚠️  Found {triplets['incomplete_triplets']} incomplete triplets - may need manual review"
        )
    if prompts["total_prompt_files"] > 0:
        print(
            f"  📝 Found {prompts['total_prompt_files']} prompt files - consider analyzing for workflow insights"
        )
