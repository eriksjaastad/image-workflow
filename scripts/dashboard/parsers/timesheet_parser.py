...
class TimesheetParser:
    ...
    def parse(self) -> dict[str, Any]:
        ...
        with open(self.csv_path, encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if not date and not project_name:
                    if hours_str:
                        print(
                            f"[TIMESHEET] Row {row_num}: Skipping summary row (hours: {hours_str})"
                        )
                    continue
                ...
                try:
                    ...
                    print(
                        f"[TIMESHEET ERROR] Row {row_num}: Failed to parse hours '{hours_str}': {e}"
                    )
                    hours = 0
                ...
                if project_name:
                    ...
                    print(
                        f"[TIMESHEET] Row {row_num}: New project '{project_name}' (date: {date}, hours: {hours})"
                    )
                    ...
                    if (
                        starting_images_str
                        and not projects[current_project]["starting_images"]
                    ):
                        try:
                            projects[current_project]["starting_images"] = int(
                                starting_images_str
                            )
                            print(
                                f"[TIMESHEET]   Starting images: {starting_images_str}"
                            )
                        except ValueError as e:
                            print(
                                f"[TIMESHEET ERROR] Row {row_num}: Failed to parse starting images '{starting_images_str}': {e}"
                            )

                    # Parse final images (can be updated as project progresses)
                    if final_images_str:
                        try:
                            projects[current_project]["final_images"] = int(
                                final_images_str
                            )
                        except ValueError:
                            pass
        ...
        for proj_name, proj_data in projects.items():
            ...
            images_per_hour = (
                round(starting_images / total_hours_proj, 1)
                if total_hours_proj > 0 and starting_images > 0
                else 0
            )
            hours_per_image = (
                round(total_hours_proj / starting_images, 4)
                if starting_images > 0
                else 0
            )
            ...
            project_list.append(
                {
                    ...
                    "daily_hours": proj_data[
                        "daily_hours"
                    ],  # NEW: Include actual daily billed hours
                }
            )
        ...
        print(
            f"[TIMESHEET] Parsed {len(project_list)} projects, total {total_hours} hours"
        )

        return {
            "projects": project_list,
            "totals": {"total_hours": total_hours, "total_projects": len(project_list)},
        }
...
if __name__ == "__main__":
    ...
    for proj in data["projects"]:
        ...
        print(
            f"  Efficiency: {proj['images_per_hour']} img/hr ({proj['hours_per_image']} hr/img)"
        )
        print(
            f"  Dates: {proj['start_date']} to {proj['last_date']} ({proj['date_count']} days)"
        )
