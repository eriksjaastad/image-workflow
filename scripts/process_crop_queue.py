...
class CropQueueProcessor:
    ...
    def __init__(
        ...
        timing_simulator: HumanTimingSimulator | None = None,
    ...
    def process_batch(self, batch: dict) -> bool:
        ...
        timestamp_started = datetime.now(UTC).isoformat()
        ...
        try:
            ...
            timestamp_completed = datetime.now(UTC).isoformat()

            # Update status to completed
            self.queue_manager.update_batch_status(
                batch_id, "completed", processing_time_ms
            )
    ...
    def preflight_validation(self, batches: list[dict]) -> tuple[bool, list[str]]:
        ...
        for batch in batches:
            ...
            for crop in crops:
                ...
                if x1 < 0 or y1 < 0:
                    errors.append(
                        f"[{batch_id}] Negative crop coordinates: {crop_rect}"
                    )
                    continue
                ...
                if crop_rect_normalized:
                    if len(crop_rect_normalized) != 4:
                        errors.append(
                            f"[{batch_id}] Invalid normalized coordinates: {crop_rect_normalized}"
                        )
                        continue
                    ...
                    if not all(0 <= v <= 1 for v in [nx1, ny1, nx2, ny2]):
                        errors.append(
                            f"[{batch_id}] Normalized coords out of range [0,1]: {crop_rect_normalized}"
                        )
                        continue
                    ...
                    if nx1 >= nx2 or ny1 >= ny2:
                        errors.append(
                            f"[{batch_id}] Invalid normalized dimensions: {crop_rect_normalized}"
                        )
                        continue
                ...
                if not dest_directory.exists():
                    warnings.append(
                        f"[{batch_id}] Destination will be created: {dest_directory}"
                    )
                elif not dest_directory.is_dir():
                    errors.append(
                        f"[{batch_id}] Destination exists but is not a directory: {dest_directory}"
                    )
                ...
                if dest_file.exists():
                    warnings.append(
                        f"[{batch_id}] Will overwrite existing file: {dest_file}"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Process crop queue with human-like timing"
    )
    parser.add_argument(
        "--fast", action="store_true", help="Process as fast as possible (no delays)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--preview", action="store_true", help="Preview mode (don't actually process)"
    )
    parser.add_argument(
        "--yes",
        "-y",
    ...
    if not args.fast:
        timing_file = (
            PROJECT_ROOT / "data" / "ai_data" / "crop_queue" / "timing_patterns.json"
        )
    ...
    processor = CropQueueProcessor(
        queue_manager, timing_simulator=timing_simulator, preview_mode=args.preview
    )
