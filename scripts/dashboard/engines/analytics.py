...
STANDARD_TOOL_ORDER = ["Web Image Selector", "Web Character Sorter", "Multi Crop Tool"]
...
class DashboardAnalytics:
    ...
    def generate_dashboard_response(
        ...
        print(
            f"  time_slice: {time_slice}, lookback_days: {lookback_days}, project_id: {project_id}"
        )
        print(f"{'=' * 70}")
        ...
        raw_data = self.engine.generate_dashboard_data(
            time_slice=time_slice, lookback_days=lookback_days, project_id=project_id
        )
        ...
        response: dict[str, Any] = {
            ...
            "project_comparisons": self._build_project_comparisons(
                project_metrics, raw_data, time_slice, lookback_days
            ),
            "project_kpi": self._build_project_kpi(
                project_id, project_metrics, raw_data
            ),
        ...
        step_time = time.time() - step_start
        print(
            f"[TIMING] ✓ build response metadata/projects/charts/etc: {step_time:.3f}s"
        )

        # Build detailed productivity table
        step_start = time.time()
        detailed_table = self._build_project_productivity_table(
            raw_data, project_metrics, time_slice, lookback_days
        )
        step_time = time.time() - step_start
        ...
        step_start = time.time()
        response["billed_vs_actual"] = self._build_billed_vs_actual(
            timesheet_data, project_metrics
        )
        response["billed_vs_actual_timeseries"] = (
            self._build_billed_vs_actual_timeseries(timesheet_data, project_metrics)
        )
        step_time = time.time() - step_start
        ...
        # Daily Activity Window (floating bars: offset + duration per day)
        try:
            daw = self._build_daily_activity_window(raw_data, project_id, lookback_days)
            # Attach under charts for frontend consumption
            response.setdefault("charts", {})["daily_activity_window"] = daw
        except Exception:
            response.setdefault("charts", {})["daily_activity_window"] = {
                "dates": [],
                "offset_hours": [],
                "durations": [],
            }
    ...
    def _build_daily_activity_window(
        ...
        {
          'dates': ['YYYY-MM-DD', ...],
          'offset_hours': [float],  # hours after midnight
          'durations': [float]      # hours between earliest and latest op
        }
        """
        import sqlite3
        ...
        all_ops = (
            ...
            if self._cached_file_ops is not None
            else self.engine.load_file_operations()
        )

        # Optional project filter by time window or root path if available
        projects = raw_data.get("projects", [])
        project = (
            next((p for p in projects if p.get("projectId") == project_id), None)
            if project_id
            else None
        )
        ...
        def belongs(rec: dict[str, Any]) -> bool:
            ...
            if root_hint:
                for key in ("source_dir", "dest_dir", "working_dir"):
                    ...
                    if root_hint in val:
                        return True
            # Fallback to project date window
            try:
                ...
                finished = project.get("finishedAt") if project else None
                if not started:
                    return False
                sd = datetime.fromisoformat(started.replace("Z", ""))
                ed = (
                    datetime.fromisoformat(finished.replace("Z", ""))
                    if finished
                    else datetime.now()
                )
                return sd <= dt <= ed
        ...
        # Limit by lookback window (relative to now)
        now = datetime.now()
        ...
        for rec in all_ops:
            try:
                ...
                buckets.setdefault(dkey, []).append(dt)
            except Exception:
                continue

        # Supplement with crop decision DB timestamps (earliest/last crops may precede first file-op)
        if project_id:
            try:
                db_path = (
                    self.data_dir
                ...
                if db_path.exists():
                    ...
                    cur = conn.cursor()
                    # Pull timestamps in recent window
                    cur.execute("SELECT timestamp FROM ai_decisions")
                    rows = cur.fetchall()
                    conn.close()
                    for (ts,) in rows:
                        try:
        ...
        for d in dates:
            ...
            if not times:
                offset_hours.append(0.0)
                durations.append(0.0)
                continue
            earliest = times[0]
            latest = times[-1]
            midnight = earliest.replace(hour=0, minute=0, second=0, microsecond=0)
            off = (earliest - midnight).total_seconds() / 3600.0
            dur = max((latest - earliest).total_seconds() / 3600.0, 0.0)
            # Guard against tiny gaps; round to 0.01h
            offset_hours.append(round(off, 2))
    ...
    def _build_project_comparisons(
        ...
        for project_id, metrics in project_metrics.items():
            ...
            tools_breakdown = self._build_tools_breakdown_for_project(
                project_id, project, metrics, raw_data, time_slice, lookback_days
            )
            ...
            for tool_name, tool_data in tools_breakdown.items():
                ...
                img_per_hour = (
                    round(images / hours, 1) if hours > 0 and images > 0 else 0
                )
        ...
        comparisons.sort(
            key=lambda x: x.get("startedAt", "9999-99-99")
        )  # Projects without dates go to end
    ...
    def _build_project_kpi(
        ...
        if not project_id or project_id not in project_metrics:
            ...
            iph = (
                (total_images / (total_work_minutes / 60))
                if total_work_minutes > 0
                else 0
            )

            return {"images_per_hour": round(iph, 2), "images_processed": total_images}
    ...
    def _build_project_productivity_table(
        ...
        for project in projects:
            ...
            tools_breakdown = self._build_tools_breakdown_for_project(
                project_id, project, pm, raw_data, time_slice, lookback_days
            )
    ...
    def _build_tools_breakdown_for_project(
        ...
        if self._cached_file_ops is None:
            ...
            print(
                f"[CACHE] Loaded {len(self._cached_file_ops)} file operations: {cache_time:.3f}s"
            )
        ...
        for display_name in allowed_tools_order:
            ...
            if not records:
                ...
                if display_name == "Web Image Selector":
                    ...
                    move_breakdown = (
                        by_dest.get("move", {}) if isinstance(by_dest, dict) else {}
                    )
                ...
                continue
            # Hours from file-op timing (robust per-project calculation)
            try:
                from scripts.utils.companion_file_utils import (
                    get_file_operation_metrics,
                )

                # Normalize timestamps to strings as required by the helper
                ops_for_metrics = []
                for op in records:
                    op_copy = dict(op)
            ...
            if display_name == "Multi Crop Tool":
                try:
                    ...
                    def is_cropped_dest(path: str) -> bool:
                        p = (path or "").lower()
                        return any(
                            k in p
                            for k in [
    ...
    def _build_billed_vs_actual(
        ...
        print(
            f"[BILLED_VS_ACTUAL DEBUG] Available project metrics: {list(project_metrics.keys())}"
        )
        ...
        print(
            f"[BILLED_VS_ACTUAL DEBUG] Projects with manifests: {list(projects_by_id.keys())}"
        )
        ...
        for ts_name, ts_data in timesheet_projects.items():
            ...
            for proj_id, proj_data in project_metrics.items():
                proj_normalized = (
                    proj_id.lower().replace(" ", "").replace("-", "").replace("_", "")
                )
                ...
                if ts_normalized == proj_normalized:
                    matched = True
                    print(
                        f"  - '{proj_id}' (normalized: '{proj_normalized}') → EXACT MATCH!"
                    )
                ...
                # (e.g., "mojo-1" should match "mojo1" but "mojo1-4/mojo-3" should NOT match "mojo1")
                    ...
                    if idx != -1:
                        ...
                        if before_ok and after_ok:
                            matched = True
                            print(
                                f"  - '{proj_id}' (normalized: '{proj_normalized}') → PARTIAL MATCH with boundaries"
                            )
                        else:
                            print(
                                f"  - '{proj_id}' (normalized: '{proj_normalized}') → substring found but NOT a word boundary (rejected)"
                            )
                else:
                    print(
                        f"  - '{proj_id}' (normalized: '{proj_normalized}') → no match"
                    )
                ...
                if matched:
                    ...
                    print(
                        f"  ✓ MATCHED! Project: {proj_id}, work_hours: {actual_hours}, started_at: {started_at}"
                    )
                    break
        ...
        for c in comparison[:3]:  # Show first 3 as sample
            print(
                f"  - {c['name']}: billed={c['billed_hours']}h, actual={c['actual_hours']}h, match={c['matched_project']}, started={c.get('started_at')}"
            )
    ...
    def _build_billed_vs_actual_timeseries(
        ...
        try:
            ...
            print(
                f"[BILLED_VS_ACTUAL_TS DEBUG] Processing {len(timesheet_projects)} timesheet projects"
            )
        ...
        for ts_proj in timesheet_projects:
            try:
                ...
                daily_billed_hours = ts_proj.get(
                    "daily_hours", {}
                )  # NEW: Get actual daily billed hours

                print(
                    f"[BILLED_VS_ACTUAL_TS DEBUG] Project '{ts_name}' has {len(daily_billed_hours)} days with billed hours"
                )
                ...
                if not matched_project:
                    print(
                        f"[BILLED_VS_ACTUAL_TS DEBUG] Skipping '{ts_name}' (no manifest match)"
                    )
                    continue

                print(
                    f"[BILLED_VS_ACTUAL_TS DEBUG] Processing '{ts_name}' → matched to '{matched_project}'"
                )
                print(
                    f"  Billed: {total_billed_hours}h over {date_count} days ({start_date_str} to {last_date_str})"
                )
                ...
                if (
                    not hasattr(self, "_cached_file_ops_for_daily")
                    or self._cached_file_ops_for_daily is None
                ):
                    self._cached_file_ops_for_daily = self.engine.load_file_operations()
                    print(
                        f"  Loaded {len(self._cached_file_ops_for_daily)} file operations"
                    )
                ...
                for date_key in sorted(ops_by_day.keys()):
                    ...
                    try:
                        ...
                        print(
                            f"    Warning: Failed to calculate metrics for {date_key}: {e}"
                        )
                ...
                for date_key in sorted(daily_work_hours.keys()):
                    ...
                    if work_hours > 0:
                        ...
                        try:
                            ...
                            timesheet_date_key = dt.strftime(
                                "%-m/%-d/%Y"
                            )  # macOS/Linux format (no leading zeros)
                        except Exception:
                            timesheet_date_key = None

                        billed_hours_for_day = daily_billed_hours.get(
                            timesheet_date_key, 0
                        )

                        if billed_hours_for_day > 0:
                            dates.append(date_key)
                            billed_by_day.append(
                                round(billed_hours_for_day, 2)
                            )  # ACTUAL daily billed hours!
                            actual_by_day.append(round(work_hours, 2))
                        else:
                            print(
                                f"  Warning: No billed hours found for {date_key} (timesheet key: {timesheet_date_key})"
                            )
                ...
                print(
                    f"  Daily billed sum: {billed_sum:.2f}h vs Timesheet total: {total_billed_hours}h"
                )
                print(
                    f"  Daily actual sum: {daily_sum:.2f}h vs Project total: {total_work_hours}h"
                )
                ...
                if daily_sum > 0 and abs(daily_sum - total_work_hours) > 0.5:
                    ...
                    print(
                        f"  Applying scale factor {scale_factor:.3f} to match project total"
                    )
                ...
                print(
                    f"[BILLED_VS_ACTUAL_TS ERROR] Failed to process '{ts_proj.get('name', 'unknown')}': {e}"
                )
                import traceback
        ...
        print(
            f"[BILLED_VS_ACTUAL_TS DEBUG] Built daily data for {len(daily_data)} projects"
        )
    ...
    def _load_timesheet_data(self) -> dict[str, Any]:
        ...
        """
        print(
            f"[TIMESHEET DEBUG] Loading timesheet from: {self.timesheet_parser.csv_path}"
        )
        try:
            if not self.timesheet_parser.csv_path.exists():
                print(
                    f"[TIMESHEET ERROR] CSV file does not exist: {self.timesheet_parser.csv_path}"
                )
                return {
                    "projects": [],
            ...
            print(
                f"[TIMESHEET DEBUG] Loaded {len(data.get('projects', []))} projects, total hours: {data.get('totals', {}).get('total_hours', 0)}"
            )
            print(
                f"[TIMESHEET DEBUG] First 3 projects: {[p['name'] for p in data.get('projects', [])[:3]]}"
            )
            return data
        except Exception as e:
            import traceback
    ...
    def _build_queue_stats(self, lookback_days: int) -> dict[str, Any]:
        ...
        try:
            ...
            throughput_data = [
                throughput_by_day[date]["crops"] for date in throughput_labels
            ]
...
def main():
    ...
    print(
        f"✅ Baseline labels (D): {len(data['metadata']['baseline_labels']['D'])} days"
    )
    ...
    output_file = (
        project_root / "data" / "dashboard_cache" / "dashboard_analytics_sample.json"
    )
